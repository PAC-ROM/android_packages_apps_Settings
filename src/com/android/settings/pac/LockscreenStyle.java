...
public class LockscreenStyle extends SettingsPreferenceFragment
    ...
    private void updateLockSummary() {
        ...
        } else if (value.contains("pac_lock")) {
            resId = R.string.lockscreen_lock_icon_pac;
    ...
    private void resizePacLock() {
        Bitmap pacLock = BitmapFactory.decodeResource(getResources(), R.drawable.pac_lock);
        if (pacLock != null) {
            String path = null;
            int px = requestImageSize();
            pacLock = Bitmap.createScaledBitmap(pacLock, px, px, true);
            try {
                mLockImage.createNewFile();
                mLockImage.setWritable(true, false);
                File image = new File(getActivity().getFilesDir() + File.separator
                            + "pac_lock" + System.currentTimeMillis() + ".png");
                path = image.getAbsolutePath();
                mLockImage.renameTo(image);
                FileOutputStream outPut = new FileOutputStream(image);
                pacLock.compress(Bitmap.CompressFormat.PNG, 100, outPut);
                image.setReadable(true, false);
                outPut.flush();
                outPut.close();
            } catch (Exception e) {
