...
public class ReportingServiceManager extends BroadcastReceiver {
    ...
    public static void setAlarm(Context context) {
        ...
        if (prefs.contains(PACStats.ANONYMOUS_OPT_IN)) {
        ...
        long lastSynced = prefs.getLong(PACStats.ANONYMOUS_LAST_CHECKED, 0);
        if (lastSynced == 0) {
    ...
    private static void migrate(Context context, SharedPreferences prefs) {
        Utilities.setStatsCollectionEnabled(context,
                prefs.getBoolean(PACStats.ANONYMOUS_OPT_IN, true));
        prefs.edit().remove(PACStats.ANONYMOUS_OPT_IN).commit();
    }
