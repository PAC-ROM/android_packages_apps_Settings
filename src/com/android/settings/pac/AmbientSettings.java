...
public class AmbientSettings extends SettingsPreferenceFragment implements
    ...
    private void updateDozeListMode() {
        final boolean pocketMode = Settings.PAC.getInt(getContentResolver(),
                   Settings.PAC.DOZE_POCKET_MODE, 0) != 0;
        final boolean shakeMode = Settings.PAC.getInt(getContentResolver(),
                   Settings.PAC.DOZE_SHAKE_MODE, 0) != 0;
        final boolean pulseMode = Settings.PAC.getInt(getContentResolver(),
                   Settings.PAC.DOZE_PULSE_ON_NOTIFICATIONS, 1) != 0;
        int listMode = 6;
    ...
    private void appliedAccTest() {
        Settings.PAC.putInt(getContentResolver(),
                    Settings.PAC.DOZE_SHAKE_ACC_THRESHOLD, mAccValue);
    ...
    private void enableShakeThreshold(boolean enabled) {
        final boolean writeMode = Settings.PAC.getInt(getContentResolver(),
                   Settings.PAC.DOZE_OVERWRITE_VALUE, 0) != 0;
    ...
    private void updateDozeListModeValue(int listMode) {
        if (listMode == 1) {
            ...
            Settings.PAC.putInt(getContentResolver(),
                   Settings.PAC.DOZE_POCKET_MODE, 1);
            Settings.PAC.putInt(getContentResolver(),
                   Settings.PAC.DOZE_SHAKE_MODE, 1);
            Settings.PAC.putInt(getContentResolver(),
            ...
            Settings.PAC.putInt(getContentResolver(),
                   Settings.PAC.DOZE_SHAKE_MODE, 1);
            Settings.PAC.putInt(getContentResolver(),
                   Settings.PAC.DOZE_PULSE_ON_NOTIFICATIONS, 0);
        } else if (listMode == 3) {
            enableShakeThreshold(false);
            enabledTimeMode(true);
            Settings.PAC.putInt(getContentResolver(),
    ...
    private void updateDozeOptions() {
        ...
        if (mDozeShakeThreshold != null) {
            mAccValue = Settings.PAC.getInt(getContentResolver(),
                    Settings.PAC.DOZE_SHAKE_ACC_THRESHOLD, 10);
            mDozeShakeThreshold.setValue(String.valueOf(mAccValue));
            int index = mDozeShakeThreshold.findIndexOfValue(String.valueOf(mAccValue));
            if (index != -1) {
                mDozeShakeThreshold.setSummary(mDozeShakeThreshold.getEntries()[index]);
    ...
    public boolean onPreferenceChange(Preference preference, Object objValue) {
        ...
        if (preference == mDozePulseVisible) {
            ...
            Settings.PAC.putInt(getContentResolver(),
                    Settings.PAC.DOZE_PULSE_DURATION_VISIBLE, dozePulseVisible);
        }
        if (preference == mDozePulseOut) {
            int dozePulseOut = (Integer) objValue;
            Settings.PAC.putInt(getContentResolver(),
                    Settings.PAC.DOZE_PULSE_DURATION_OUT, dozePulseOut);
        }
