...
public class WindowSettings extends Fragment implements OnSeekBarChangeListener, OnColorChangedListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        SETTINGS_SYSTEM = Settings.System.APP_WINDOW;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedinstanceState) {
        ...
        mAnimationType.setKey(SETTINGS_SYSTEM[AokpRibbonHelper.WINDOW_ANIMATION]);
        ...
        mWindowColor.setImageBitmap(getPreviewBitmap(Settings.System.getInt(mContext.getContentResolver(), SETTINGS_SYSTEM[AokpRibbonHelper.WINDOW_COLOR], Color.BLACK)));
        mWindowColor.setOnClickListener(new Button.OnClickListener() {
            @Override
            public void onClick(View v) {
                mColorCounter = 1;
                ColorPickerDialog picker = new ColorPickerDialog(mContext, Settings.System.getInt(mContext.getContentResolver(), SETTINGS_SYSTEM[AokpRibbonHelper.WINDOW_COLOR], Color.BLACK));
        ...
        mAnimationDuration.setProgress(Settings.System.getInt(mContext.getContentResolver(), SETTINGS_SYSTEM[AokpRibbonHelper.WINDOW_ANIMATION_DURATION], 50));
        mAnimationDuration.setOnSeekBarChangeListener(this);

        mWindowSize = (SeekBar) main.findViewById(R.id.window_size);
        mWindowSize.setProgress(Settings.System.getInt(mContext.getContentResolver(), SETTINGS_SYSTEM[AokpRibbonHelper.WINDOW_SIZE], 30));
        mWindowSize.setOnSeekBarChangeListener(this);

        mWindowSpace = (SeekBar) main.findViewById(R.id.window_space);
        mWindowSpace.setProgress(Settings.System.getInt(mContext.getContentResolver(), SETTINGS_SYSTEM[AokpRibbonHelper.WINDOW_SPACE], 5));
    ...
    public void onStopTrackingTouch(SeekBar seekBar) {
        if (seekBar == mAnimationDuration) {
            Settings.System.putInt(mContext.getContentResolver(), SETTINGS_SYSTEM[AokpRibbonHelper.WINDOW_ANIMATION_DURATION], mSeekBarProgress);
        } else if (seekBar == mWindowSpace) {
            Settings.System.putInt(mContext.getContentResolver(), SETTINGS_SYSTEM[AokpRibbonHelper.WINDOW_SPACE], mSeekBarProgress);
        } else if (seekBar == mWindowSize) {
            Settings.System.putInt(mContext.getContentResolver(), SETTINGS_SYSTEM[AokpRibbonHelper.WINDOW_SIZE], mSeekBarProgress);
        }
    }
    ...
    public void onColorChanged(int color) {
            ...
            Settings.System.putInt(mContext.getContentResolver(), SETTINGS_SYSTEM[AokpRibbonHelper.WINDOW_COLOR], color);
    }
